...
public final class AzureVMCloudVerificationTask extends AsyncPeriodicWork {
    ...
    public static void verify(String cloudName, String templateName) {
        ...
        if (agentTemplate == null) {
            LOGGER.log(
                    getStaticNormalLoggingLevel(),
                    "AzureVMCloudVerificationTask: verify: " + "could not retrieve agent template named {0} in {1}",
                    new Object[] {templateName, cloudName});
            return;
        }
    ...
    private static void verifyCloud(String cloudName) {
        ...
        LOGGER.log(
                getStaticNormalLoggingLevel(), "AzureVMCloudVerificationTask: verify: verifying cloud {0}", cloudName);
        ...
        synchronized (cloud) {
            ...
            if (verifyConfiguration(cloud)) {
                LOGGER.log(
                        getStaticNormalLoggingLevel(),
                        "AzureVMCloudVerificationTask: validate: {0} " + "verified pass",
                        cloudName);
                // Update the count
                updateCloudVirtualMachineCounts(cloud);
    ...
    public static boolean verifyConfiguration(AzureVMCloud cloud) {
        ...
        String result = cloud.getServiceDelegate()
                .verifyConfiguration(
        ...
        synchronized (cloud) {
            ...
            try {
                ...
                final Map<String, Integer> counts =
                        sd.getVirtualMachineCountsByTemplate(cloud.getCloudName(), cloud.getResourceGroupName());
